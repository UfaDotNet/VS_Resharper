...
namespace Sparrow.Logging
{
    public sealed class LoggingSource
    {
        ...
        private static readonly byte[] _headerRow =
            Encoding.UTF8.GetBytes($"Time,\tThread,\tLevel,\tSource,\tLogger,\tMessage,\tException{Environment.NewLine}");
        ...
        public async Task Register(WebSocket source, WebSocketContext context, CancellationToken token)
        {
            ...
            while (token.IsCancellationRequested == false)
            {
                ...
                if (maxBytes > arraySegment.Count)
                    arraySegment = new ArraySegment<byte>(new byte[(int) Math.Pow(maxBytes, 2)]);
        ...
        private void BackgroundLogger()
        {
            try
            {
                ...
                var threadStatesToRemove = new Stack<WeakReference<LocalThreadWriterState>>();
                while (this._keepLogging)
                {
                    try
                    {
                        var maxFileSize = this.MaxFileSizeInBytes;
                        if (this.TryGetNewStreamAndApplyRetentionPolicies(maxFileSize, out var currentFile) == false)
                        {
                            if (this._keepLogging == false)
                                return;
                            this._hasEntries.Wait(1000);
                            continue;
                        }
                        using (currentFile)
                        {
                            this._readyToCompress.Set();
                            ...
                            while (sizeWritten < maxFileSize)
                            {
                                if (foundEntry == false)
                                {
                                    ...
                                    if (this._hasEntries.IsSet == false)
                                    {
                                    ...
                                    this._hasEntries.Wait();
                                    if (this._keepLogging == false)
                                        return;

                                    this._hasEntries.Reset();
                                }
                                ...
                                foreach (var threadStateRef in threadStates)
                                {
                                    ...
                                    for (var i = 0; i < 16; i++)
                                    {
                                        ...
                                        sizeWritten += this.ActualWriteToLogTargets(item, currentFile);

                                        threadState.Free.Enqueue(item);
                                    }
                                }

                                while (threadStatesToRemove.Count > 0)
                                {
                                    threadStates.Remove(threadStatesToRemove.Pop());
                                }

                                if (this._newThreadStates.IsEmpty)
                                    continue;

                                while (this._newThreadStates.TryDequeue(out WeakReference<LocalThreadWriterState> result))
                                    threadStates.Add(result);

                                this._hasEntries.Set(); // we need to start writing logs again from new thread states
                            }
                        }
                    }
                    catch (OutOfMemoryException)
                    {
                            ...
                            this._hasEntries.Wait(time))
                        {
                            ...
                            foreach (var newThreadState in this._newThreadStates)
                            {
            ...
            {
                this._readyToCompress.Set();
                if(this._compressLoggingThread?.Join(1000) == false)
            ...
            while (true)
            {
                try
                {
                    ...
                    for (var i = 0; i < logFiles.Length - 1; i++)
                    {
                        ...
                        try
                        {
                            using (var logStream = File.Open(logFile, FileMode.Open, FileAccess.Read))
                            {
                                ...
                                using (var newFileStream = File.Open(newZippedFile, FileMode.Create, FileAccess.Write))
